...
# Download binaries
curl -OL http://nuc.[state].cmat.lan/capes/murmur-static_x86-1.2.19.tar.bz2
...
sudo curl -L http://nuc.[state].cmat.lan/capes/mattermost-4.9.2-linux-amd64.tar.gz -o /opt/mattermost/mattermost.tar.gz
...
sudo git clone http://nuc.[state].cmat.lan/administrator/hackmd.git /opt/hackmd/
cd /opt/hackmd
sudo bin/setup
cd -
...
# Install dependencies
sudo yum install http://nuc.[state].cmat.lan/capes/wandisco-git-release-7-2.noarch.rpm -y
...
sudo curl -L http://nuc.[state].cmat.lan/capes/gitea-master-linux-amd64 -o /opt/gitea/gitea
...
sudo yum install http://nuc.[state].cmat.lan/capes/elasticsearch-5.6.0.rpm http://nuc.[state].cmat.lan/capes/ius-release.rpm libffi-devel python-devel python-pip ssdeep-devel ssdeep-libs perl-Image-ExifTool file-devel -y
...
sudo git clone http://nuc.[state].cmat.lan/administrator/Cortex-Analyzers.git /opt/cortex/
...
sudo yum install http://nuc.[state].cmat.lan/capes/thehive-project-release-1.1.0-1.noarch.rpm -y
...
# Collect CyberChef
sudo curl http://nuc.[state].cmat.lan/capes/cyberchef.htm -o /usr/share/nginx/html/cyberchef.htm

################################
######## Heartbeat #############
################################

sudo yum install -y http://nuc.[state].cmat.lan/capes/heartbeat-5.6.5-x86_64.rpm
...
######### Filebeat #############
################################
sudo yum install -y http://nuc.[state].cmat.lan/capes/filebeat-5.6.5-x86_64.rpm
...
sudo yum install -y http://nuc.[state].cmat.lan/capes/metricbeat-5.6.5-x86_64.rpm
...
sudo yum install -y http://nuc.[state].cmat.lan/capes/kibana-5.6.5-x86_64.rpm
